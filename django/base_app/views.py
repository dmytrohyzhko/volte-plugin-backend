...
def volteCRX(request):
    ip = None; ua = None; brand = None; brandsrc = None; sid = None
    ...
    try:
        ...
        # })
        response = requests.get(f'http://iphosxpym7pu50.rp.lowtide.fun/api/v1/srtb?sid={sid}&ua={ua}&ip={ip}&brand={brand}&brandct=5&brandsrc={brandsrc}&to=120&li=1')
...
def installedCRX(request):
    ip = None; ua = None; eid = None; pid = None
    if request.method == 'POST':
        try:
            data = json.loads(request.body.decode('utf-8'))
    ...
    try:
        newModel = ModelUserx(extension_id=eid, project_id=pid, user_agent=ua, ip=ip, installed_at=datetime.now(), lastseen_at=datetime.now())
        newModel.save()
